...
class BridgeHandler:
    ...
    async def _update_hija(self, hija_id: str, status: str, result: Optional[Dict] = None, error: Optional[str] = None):
    ...
    async def audit_full(self, context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            try:
                manifestator_url = settings.manifestator_url or f"http://manifestator:{settings.manifestator_port}"
    ...
    async def organize(self, intent: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if not patch_plan.get("operations"):
                await self._update_hija(hija.hija_id, "completed", result={"patch": patch_plan, "applied": False})
                return {"hija_id": hija.hija_id, "patch": patch_plan, "applied": False}

            backup_root = Path(patch_plan.get("backup_root", self.repo_root / "build" / "artifacts" / "backups"))
            loop = asyncio.get_event_loop()
            apply_result = await loop.run_in_executor(
                None,
                lambda: apply_patch_operations(
                    patch_plan.get("operations", []),
                    self.repo_root,
                    backup_root,
                    ttl_seconds=60,
                ),
    ...
    async def scan_hive(self, context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
    ...
    async def spawn_cmd(self, cmd: str, context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
