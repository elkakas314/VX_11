        ...
        if req.require.get("switch", False):
            try:
                ...
                async with httpx.AsyncClient(timeout=30.0) as client:
                    ...
                    write_log(
                        "madre", f"vx11_intent:switch_url:{switch_url}:{correlation_id}"
                    )
                    ...
                    write_log(
                        "madre",
                        f"vx11_intent:switch_response:{sresp.status_code}:{correlation_id}",
                    )
                    if sresp.status_code == 200:
                        ...
                        write_log(
                            "madre",
                            f"vx11_intent:switch_upstream_error:{correlation_id}:{sresp.status_code}:body={sresp.text[:200]}",
                        )
                        # Degrade to fallback instead of returning error
            except Exception as e:
                write_log(
                    "madre",
                    f"vx11_intent:switch_exception:{correlation_id}:{str(e)[:200]}",
                )
                # Degrade to fallback instead of returning error
