...
# VX11 Agent Bootstrap v2.4 — PROTOCOLO QUIRURGICO + STATUS_PACK

**🎯 OBJETIVO:** Cada vez que uses `@vx11`, Copilot aplica protocolo quirúrgico automáticamente.

**⚓ BOOTSTRAP OBLIGATORIO** (ejecutar ANTES que nada):
1. **`.github/VX11_STATUS_PACK.md`** ← Fuente única de verdad (invariantes, puertos, rutas, DeepSeek R1 default rule)
2. **`.github/agents/vx11.agent.md`** ← Este archivo (protocol quirúrgico)
3. **`.github/copilot-instructions.md`** ← Copilot rules (modelos, prohibiciones, coste)
4. **`docs/audit/CLEANUP_EXCLUDES_CORE.txt`** ← Hard-exclude paths

**Última actualización:** 2025-12-30T22:10:00Z  
**Status:** ✅ OPERATIONAL (full-test profile: madre, tentaculo_link, switch, operator-backend, operator-frontend, spawner) + 🔬 QUIRURGICO PERSISTENTE + 📋 STATUS_PACK_INTEGRATED

---

## ⚡ COMPORTAMIENTO AUTOMÁTICO (Cada Invocación)
- Endpoints operator/spawner integrados
- docker-compose.full-test.yml debe pasar healthcheck
...
2. Cambio mínimo (una cosa, nunca refactor)
...
2. DRY-RUN visible
3. Ask confirmación explícita
4. Ejecuta
5. Archiva evidencia

---

## 🔬 5 REGLAS QUIRURGICAS (NON-NEGOTIABLE)

1. **Cambios Mínimos**
   - ✂️ SOLO lo necesario, nada más
   - ❌ NO refactorings innecesarios
   - ❌ NO limpieza de código "mientras estamos"
   - ✅ Un cambio = una línea/bloque de propósito específico

2. **Auditoría Primero**
   - 📋 Analizar antes de tocar
   - 🔍 Leer contexto completo (líneas -3/+3 mínimo)
   - ✅ Validar impacto de cambio
   - 🚫 NUNCA cambiar sin entender

3. **Validación Post-Cambio**
   - ✔️ Verificar syntax/integridad post-edit
   - 📊 Ejecutar tests si aplica
   - 🔐 PRAGMA checks si toca BD
   - ✅ Health check si toca servicios

4. **Evidencia Automática**
   - 📁 Guardar en `docs/audit/<TS>/`
   - 📝 Logging de CADA cambio
   - 🔗 Traceback de decisiones
   - 🗂️ Antes/después si es relevante

5. **No Destruir**
   - 🛡️ Pre-backup antes de DELETE
   - 📦 Archivar, nunca borrar (forensic, CORE)
   - 🔄 Reversible siempre que sea posible
   - 🎯 DRY-RUN visible antes de ejecutar


### Endpoints integrados:
- /operator/api/v1/events/stream
- /operator/api/v1/spawner/
- /madre/power/maintenance/post_task

```
¿Se necesita SOLO A?
├─ SÍ → Cambio quirúrgico (mínimo)
├─ NO, también B, C... → Múltiples cambios (en paralelo)
└─ NO IMPORTA, refactoreo A+B+C → ❌ NUNCA sin pedir

¿Es destructivo (rm, DELETE, reset)?
├─ SÍ → Pre-backup + ask user + DRY-RUN + confirm
└─ NO → Procede con validación post

¿Toca BD/servicios/tokens?
├─ SÍ → Audit first + PRAGMA/health checks + post-task
└─ NO → Validation estándar

¿Es refactor/limpieza?
├─ SÍ → Ask específicamente / NUNCA "mientras estamos"
└─ NO → Quirúrgico directo
```

### Checklist Quirúrgico (ANTES de CADA cambio)

```bash
# Copiar y usar ANTES de CUALQUIER edit:

□ 1. Leer archivo completo o contexto mínimo (-3/+3 líneas)
□ 2. Entender propósito del cambio (qué, por qué, por quién)
□ 3. Identificar línea/bloque EXACTO a cambiar
□ 4. Verificar que NO hay side-effects no queridos
□ 5. Preparar DRY-RUN si es destructivo
□ 6. Si preguntas → ask user (nunca asumir)
□ 7. Hacer cambio MÍNIMO (una cosa a la vez)
□ 8. Validar post-cambio (syntax, tests, health)
□ 9. Guardar evidencia (qué, cuándo, por qué)
□ 10. Reportar cambio realizado (no esconder)
```

---

## ⚠️ INYECCIÓN AUTOMÁTICA (Copilot: APLICA SIEMPRE)

> ⚡ ESTO DEBE EJECUTARSE ANTES DE CUALQUIER ACCIÓN CON @VX11

```yaml
ON_EACH_INVOCATION:
  IF_COMANDO == "@vx11 <anything>":
    LOAD: "docs/audit/ESTILO_HAIKU_4_5_PORTABLE.md" (protocolo quirúrgico)
...
**¿Qué significa esto?**
- Cada vez que invocas `@vx11`, estas 5 reglas se aplican automáticamente
...
- Flujo obligatorio: (1) Auditoria primero, (2) Cambios mínimos, (3) POST /madre/power/maintenance/post_task + evidencia.
...
- **LIMPIEZA:** Antes de `rm/rmdir/mover` → cargar `docs/audit/CLEANUP_EXCLUDES_CORE.txt` y validar que no sea path CORE.
- **CAMBIOS QUIRURGICOS:** SIEMPRE que se pida editar código: solo lo mínimo, audit-first, validar post, guardar evidencia. **PORTABLE a ANY modelo (GPT-5, Mini, Raptor, etc.)**
...
**Ver tasks en ejecución (loop cada 2 seg):**
```bash
...
sqlite3 data/runtime/vx11.db \
  "SELECT provider_id, tokens_used_today, daily_limit_tokens, \
          ...
          CASE WHEN tokens_used_today >= daily_limit_tokens THEN '🔴 EXHAUSTED' ELSE '🟢 OK' END as status \
   FROM cli_providers WHERE enabled=1 ORDER BY pct_used DESC;"
```

**Token history (últimas 7 días):**
```bash
sqlite3 data/runtime/vx11.db \
...
**Listar backups (más nuevos primero):**
```bash
...
# 2. Validar que nada sea path crítico
CANDIDATE="/ruta/a/limpiar"
if echo "$EXCLUDE" | grep -q "$CANDIDATE"; then
  echo "❌ ABORTAR: $CANDIDATE está en CLEANUP_EXCLUDES_CORE.txt"
  exit 1
fi
echo "✓ $CANDIDATE no está en exclusiones"
...
echo "[CLEANUP] Borrando logs > $DAYS_OLD días"
...
echo "✓ Limpieza completada. Evidencia: docs/audit/$TS/cleanup_logs*.txt"
```

#### 🟠 Archivar Forensic Crashes (SAFE)
...
if grep -q "forensic/crashes" docs/audit/CLEANUP_EXCLUDES_CORE.txt; then
  echo "✓ forensic/crashes está protegido"
...
#### 🔴 Rotar Backups (Conservar 2 Nuevos)

```bash
#!/bin/bash
TS=$(date -u +%Y%m%dT%H%M%SZ)

echo "[CLEANUP] Rotación de backups"
...
for ((i=2; i<TOTAL; i++)); do
  ...
  if echo "$EXCLUDE" | grep -q "$(basename $BACKUP)"; then
    echo "⚠️  Saltando (CORE protegido): $BACKUP"
    continue
  fi
...
#### 🔴🔴 Limpiar Audit Antiguos (AGGRESSIVE — Pedir Confirmación)
...
echo "⚠️  AGGRESSIVE: Borrando audits > $DAYS_OLD días"
...
echo "Se borraría: ~$WOULD_DELETE carpetas"
echo ""
...
if [[ $REPLY =~ ^[Ss]$ ]]; then
  ...
  echo "✓ Limpieza completada. Evidencia: docs/audit/$TS/cleanup_audit_aggressive.txt"
else
  echo "Cancelado."
fi
```

#### 🔴🔴🔴 Vaciar Tabla de BD (SURGICAL — Requiere Confirmación Explícita)
...
if [ -z "$TABLE" ]; then
  ...
  echo "⚠️  NUNCA vaciar: spawns, daughters, incidents, module_status"
  exit 1
fi
...
if [ "$CONFIRM" = "sí" ]; then
  ...
  echo "✓ Tabla vaciada. Evidencia: docs/audit/$TS/delete_$TABLE.log"
...
echo "[POST-CLEANUP] Validación final"
...
if [ $HEALTH -eq 2 ]; then
  ...
  echo "⚠️  Check manual: curl -s http://localhost:8001/health"
fi
...
echo "✓ Post-cleanup completada. Evidencia: docs/audit/$TS/"
```

---

## Quick Commands (Atajos — Copia y Pega)
...
**Reporte de reparación reciente:**
```bash
...
**Post-task maintenance (DESPUÉS de cualquier cambio):**
```bash
curl -X POST http://localhost:8001/madre/power/maintenance/post_task \
  -H "Content-Type: application/json" \
  -d '{}' | jq '.'
...
## Route Manifest (Rutas Críticas + Propósito)

| Ruta | Qué Es | Úsalo Para |
|------|---------|-----------|
...
| `docs/audit/DB_SCHEMA_v7_FINAL.json` | Schema en JSON | Parse programático |
| `docs/audit/PERCENTAGES.json` | Score de estabilidad (0-100%) | Ver salud del sistema |
| `docs/audit/SCORECARD.json` | Métricas de tamaño BD | Monitorear crecimiento |
| `docs/audit/` | Evidencia por timestamp | Auditoría de cambios |
| `VX11_CONTEXT.md` | Referencia rápida del sistema | Overview en 5 min |
| `.github/agents/vx11.agent.md` | ESTE ARCHIVO | Tu bootstrap |
| `forensic/crashes/` | Dumps de fallos | Debug de crashes |
| `docker-compose.yml` | Configuración de servicios | Ver puertos, vars |

### Documentos Generados Automáticamente (Post-Task)

```
docs/audit/<TIMESTAMP>/
├── db_quick_check.txt           (PRAGMA quick_check output)
├── db_integrity_check.txt       (PRAGMA integrity_check output)
├── db_fk_check.txt              (Foreign keys check)
├── counts.json                  (Conteos de registros por tabla)
├── health_8000.json             (Health de cada servicio)
├── health_8001.json
├── ... (uno por cada puerto)
├── madre_power_response.json    (Response de maintenance)
├── percentages_snapshot.json    (Estabilidad en ese momento)
└── scorecard_snapshot.json      (Métricas BD en ese momento)
```
...
-- Si EXHAUSTED → espera reset_hour_utc
```
...
[ $HEALTHY -eq 10 ] && { echo "✓ $HEALTHY/10"; ((PASS++)); } || { echo "✗ $HEALTHY/10"; ((FAIL++)); }
...
[ $DB_PASS -eq 3 ] && { echo "✓ $DB_PASS/3"; ((PASS++)); } || { echo "✗ $DB_PASS/3"; ((FAIL++)); }

# 3. MADRE power endpoint
echo -n "MADRE power endpoint: "
curl -s http://localhost:8001/openapi.json | jq -e '.paths["/madre/power/maintenance/post_task"]' >/dev/null 2>&1 && { echo "✓"; ((PASS++)); } || { echo "✗"; ((FAIL++)); }
...
[ "$OFF" -eq 0 ] && { echo "✓"; ((PASS++)); } || { echo "✗ $OFF modules off"; ((FAIL++)); }

echo
echo "[PRE-ACTION] Result: $PASS passed, $FAIL failed"
[ $FAIL -eq 0 ] && echo "✓ PROCEEDING" || echo "✗ BLOCKING - fix issues first"
```
...
```
1. POST /spawn
   ↓ validate (ttl >= 30, max_retries >= 0)
   ↓ INSERT spawns (status='pending')
   ↓ POST /spawner/spawn
   ↓ daughter created, INSERT daughters
   ↓ daughter executes
   ↓ if error AND attempt < max_retries: apply mutation, retry
   ↓ daughter POST /madre/callback {result}
   ↓ madre UPDATE spawns (status='completed')
...
```
hermes POST /execute → subprocess.run → MADRE callback
...
### Flow B: Task Spawn
```
POST /spawn → spawns.INSERT → daughters.INSERT → action → callback → UPDATE spawns
...
### Flow C: Auto-Repair
```
hormiguero.scan → incidents.INSERT → pheromone_log.INSERT → manifestator → drift_reports.INSERT → dsl.apply → incidents.UPDATE
...
## Post-Action Mandatory (DESPUÉS de CUALQUIER operación)
...
| Capacidad | ✓/✗ | Notas |
|-----------|-----|-------|
| Validar health + DB | ✓ | Pre-action siempre |
| Spawn tasks | ✓ | Con validation completa |
| CLI execution | ✓ | Via hermes, respeta tokens |
| Detect incidents | ✓ | Query a BD |
| Plan repairs | ✓ | Manifestator analysis |
| Run maintenance | ✓ | MADRE post_task |
| Regenerate artifacts | ✓ | Scripts canonicos |
| Modify madre code | ✗ | Escalate al operador |
| Delete spawns | ✗ | Escalate al operador |
| Recover BD corrupted | ✗ | Escalate + manual restore |
| Override rate limits | ✗ | Espera reset |
| Docker compose direct | ✗ | Usa override.yml |
...
|-----------|-----|----------|-------|
| **AUDITORÍA** | | | |
| Validar health + DB | ✓ | P0 | Pre-action siempre |
| Monitoreo real-time | ✓ | P1 | watch + Redis MONITOR |
| Detección anomalías | ✓ | P1 | TTL spreads, crash loops |
| Analysis tokens | ✓ | P1 | Hermes usage tracking |
| Performance profiling | ✓ | P2 | Query slow logs |
| **OPERACIONES** | | | |
| Spawn tasks | ✓ | P0 | Con validation completa |
| CLI execution | ✓ | P1 | Via hermes, respeta tokens |
| Detect incidents | ✓ | P0 | Query a BD |
| Plan repairs | ✓ | P1 | Manifestator analysis |
| Run maintenance | ✓ | P0 | MADRE post_task |
| Regenerate artifacts | ✓ | P1 | Scripts canonicos |
| **LIMPIEZA** | | | |
| Limpiar logs | ✓ | P2 | SAFE (> 7 días) |
| Archivar crashes | ✓ | P2 | SAFE (move a docs/audit) |
| Rotar backups | ✓ | P2 | SAFE (conserva 2 nuevos) |
| Limpiar audit old | ⚠️ | P3 | AGGRESSIVE (ask) |
| Vaciar tabla BD | ⚠️ | P3 | SURGICAL (ask + confirm) |
| **PROHIBIDO** | | | |
| Modificar madre core | ✗ | N/A | Escalate al operador |
| Delete spawns/daughters | ✗ | N/A | Escalate al operador |
| Recover BD corrupted | ✗ | N/A | Escalate + manual restore |
| Override rate limits | ✗ | N/A | Espera reset |
| Docker compose direct | ✗ | N/A | Usa override.yml |
| Tocar tokens/secrets | ✗ | N/A | READ-ONLY |

---

## Toolkit Rápido (Copy-Paste Inmediato)

### 🟢 Status Instant (1 línea)
...
TS=$(date -u +%Y%m%dT%H%M%SZ); mkdir -p docs/audit/$TS; (
  ...
  echo "✓ Auditoria en: docs/audit/$TS/"
)
```

### 🟢 Spawn Quick Task (edit payload)
...
### 🟡 Cleanup Safe (logs + forensic + backups)
...
|------------|---------|------|
| Status general | `curl -s http://localhost:8000/health` | 🟢 |
| Spawn a task | `curl -X POST http://localhost:8008/spawn -d '...'` | 🟢 |
| Post-task | `curl -X POST http://localhost:8001/madre/power/maintenance/post_task` | 🟢 |
| Ver spawns pending | `sqlite3 data/runtime/vx11.db "SELECT spawn_id, status FROM spawns WHERE status='pending' LIMIT 10;"` | 🟢 |
| Ver incidentes | `sqlite3 data/runtime/vx11.db "SELECT * FROM incidents ORDER BY detected_at DESC LIMIT 5;"` | 🟢 |
| Token usage | `sqlite3 data/runtime/vx11.db "SELECT provider_id, tokens_used_today FROM cli_providers;"` | 🟢 |
| Ver evidencia | `ls -la docs/audit/` | 🟢 |
| DB check | `sqlite3 data/runtime/vx11.db "PRAGMA integrity_check;"` | 🟢 |
| Limpiar logs | `find logs -type f -mtime +7 -delete` | 🟡 |
| Rotar backups | `ls -t data/backups/vx11*.db \| tail -n +3 \| xargs mv ... archived/` | 🟡 |

---

## Archivos Esenciales (No Tocar Sin Permiso)

- `data/runtime/vx11.db` → BD principal (solo query + backup)
- `docker-compose.yml` → Config servicios (usar .override)
- `tokens.env` → Secretos (READ-ONLY)
- `madre/main.py` → Orchestrator core (code review required)
- `docs/audit/CLEANUP_EXCLUDES_CORE.txt` → Validar antes de rm/rmdir
...
└─ Tokens: Análisis de Tokens

OPERACIÓN
├─ Spawn: Spawn Quick Task
├─ Post-action: Full Audit
├─ Health check: Quick Reference
└─ Maintenance: MADRE post_task

---

## POWER CONTROL (container-level) — OBLIGATORIO

**Rail**: Container-level ONLY. NO `docker exec` ni `kill` ni procesos internos. SOLO `docker compose`.

**Servicios canónicos (allowlist)**:
- tentaculo_link (8000)
- madre (8001) — PROTECTED, NO stop
- switch (8002)
- hermes (8003)
- hormiguero (8004)
- mcp (8006)
...
**Formato de salida (FIXED)**:
```json
{
  "status": "ok|error|plan",
  "policy": "solo_madre|low_power|operative_core|full",
  "executed": [
    {
      "rc": 0,
      "cmd": ["docker", "compose", "-p", "vx11", "stop", "switch"],
      "stdout_path": "docs/audit/madre_power_stop_switch_*/cmd_0_stdout.txt",
      "elapsed_ms": 1234
    }
  ],
  "out_dir": "docs/audit/madre_power_<action>_<service>_<TS>/",
  "security_flags": {"key_ok": true, "token_ok": true, "confirm_ok": true, "rate_ok": true}
}
```

**Comandos permitidos**:
1. `POST /madre/power/policy/solo_madre/apply` — Parar TODOS menos madre
2. `GET /madre/power/policy/solo_madre/status` — Verificar si solo madre está arriba
3. `POST /madre/power/service/start {"service":"switch"}` — Arrancar servicio (docker compose up -d)
4. `POST /madre/power/service/stop {"service":"switch"}` — Parar servicio (docker compose stop)
5. `GET /madre/power/status` — Estado de todos los servicios (docker ps)
6. `POST /madre/power/mode` {"mode":"low_power|operative_core|full"} — Cambiar modo operativo

**DeepSeek R1 Rails** (cuando se invoque):
- PLAN PHASE: Listar cambios con pre/post snapshots
- PATCH PHASE: Aceptar SOLO docker compose commands, rechazar docker exec / kill / process-level
- VALIDATION: Verificar post-action con health checks + docker ps
- EVIDENCE: Guardar en OUTDIR con timestamps + correlation_id

---

LIMPIEZA (Pedir Confirmación)
├─ Logs: find logs -mtime +7 -delete
├─ Crashes: Archivar Forensic Crashes
├─ Backups: Rotar Backups
├─ Audit old: Limpiar Audit Antiguos
└─ BD tabla: Vaciar Tabla BD
```

---

**VX11 BOOTSTRAP v2.1 — OPERACIONAL**

**Última actualización:** 2025-12-24T21:49:31Z  
**Status:** ✅ Todos sistemas UP (9/9), BD OK, autonomy 100%  
**Versión:** v2.1 (Enhanced Tools + Limpieza Quirurgica)  
**Capabilidad:** Auditoría → Operación → Limpieza (SAFE/AGGRESSIVE/SURGICAL)
