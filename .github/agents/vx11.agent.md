...
| Regla | Resultado |
|---|---|
| Formato flexible | Respuestas concisas; usar tabla solo si aporta claridad |
...
| 2 | Detectar endpoints vivos (openapi.json) | API discovery (resumen) |
| 3 | Scan BD copilot_* | VX11_AGENT_BOOTSTRAP_REPORT.md |
| 4 | Snapshots canónicos | data/backups/vx11_CANONICAL_* |
| 5 | CPU/RAM/zombies/ext | Resumen diagnóstico |
...
|---|---|---|
| `status` | `vx11_workflow_runner.py status` | Resumen servicios + recursos |
| `cleanup:diagnose` | Diagnóstico CPU/RAM/zombies/ext (lectura) | Resumen hallazgos |
| `task:enqueue "<t>"` | `vx11_task_router.py enqueue` | task_id + TARGET |
| `watch "<id>"` | `vx11_task_router.py watch` | Progreso resumido |
...
4. **Formato conciso**: preferir un resumen claro. Las tablas son opcionales y solo si añaden claridad.
...
2. Resumen: Servicios (OK/BROKEN) y endpoints descubiertos
3. Resumen: CPU/RAM/Zombies
4. Resumen: Archivos modificados (git status --short)
5. Aguardar comando del usuario
```

**NUNCA** mostrar comandos. Ejecutar en VS Code terminal, capturar output, devolver un resumen conciso (tablas opcionales).
