...
class SwitchIntelligenceLayer:
    ...
    async def make_routing_decision(
        ...
        logger.info(f"Making routing decision: task_type={context.task_type}, source={context.source}")
        
        try:
            # PASO 1: Detectar tareas especiales (SYSTEM, DRIFT, AUDIO)
            if context.task_type in ("system", "workflow") or context.provider_hint == "madre":
            ...
            if context.task_type in ("drift", "audit", "manifest") or context.provider_hint == "manifestator":
                return RoutingResult(
            ...
            if context.task_type == "audio" or context.provider_hint in ("shub", "shub-audio"):
                return RoutingResult(
    ...
    async def execute_with_fallback(
        ...
        **kwargs
        ...
        try:
            ...
            write_log(
                "switch.sil",
                f"primary_engine_failed:{decision.primary_engine}:{e}"
            )
        ...
        raise Exception(f"All engines failed, including fallbacks: {decision.fallback_engines}")
