...
ROUTE_TABLE: Dict[IntentType, RouteConfig] = {
    ...
    IntentType.AUDIO: RouteConfig(
        ...
        method="POST",
        timeout=30.0,
        description="Route audio intent to Switch (canonical task pipeline)",
        auth_required=True,
    ),
    IntentType.ANALYSIS: RouteConfig(
        ...
        method="POST",
        timeout=60.0,
        description="Route analysis to Switch for intent refinement",
        auth_required=True,
    ),
    IntentType.TASK: RouteConfig(
        ...
        method="POST",
        timeout=60.0,
        description="Route task to Switch for intent refinement",
        auth_required=True,
    ),
    IntentType.SPAWN: RouteConfig(
        ...
        method="POST",
        timeout=120.0,
        description="Route spawn intent to Spawner (canonical spawn endpoint)",
        auth_required=True,
    ),
    IntentType.STREAM: RouteConfig(
        ...
        method="POST",
        timeout=300.0,
        description="Route stream intent to Switch (canonical task pipeline)",
        auth_required=True,
    ),
}
