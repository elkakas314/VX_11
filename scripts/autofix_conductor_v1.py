        ...
        if status != "ok":
            failures[test_name] = {
                ...
                "severity": (
                    "critical"
                    if status == "fail"
                    else "warning" if status == "degraded" else "timeout"
                ),
            }
...
async def retest_after_fix(outdir: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"status": "timeout", "message": "E2E test timeout (300s)"}
    except Exception as e:
        return {"status": "error", "message": str(e)}


async def run_autofix_cycle(
    failure_context: str, cycle: int, outdir: Path
) -> Dict[str, Any]:
    ...
    if patch_result.get("status") != "applied":
        ...
        return {"cycle": cycle, "status": "patch_failed", "result": patch_result}
    ...
    if retest_result.get("status") == "ok":
        ...
        return {"cycle": cycle, "status": "success", "result": retest_result}
    else:
        print(f"⚠️ Tests still failing after fix")
        return {"cycle": cycle, "status": "incomplete", "result": retest_result}
...
async def main():
    ...
    parser.add_argument(
        "--test-result", help="Path to E2E test results JSON", required=False
    )
    parser.add_argument("--reason", default="autofix", help="Reason for autofix run")
    ...
    try:
        ...
        if args.test_result:
            ...
            test_dirs = list(
                audit_dir.glob("*_E2E_TEST_CONDUCTOR_v*/test_results.json")
            )
            if not test_dirs:
        ...
        autofix_results["status"] = (
            "success"
            if any(c.get("status") == "success" for c in autofix_results["cycles"])
            else "incomplete"
        )
