...
def get_table_schema(table_name: str) -> dict:
    ...
    cursor.execute(f"SELECT page_count * page_size FROM (SELECT page_count FROM pragma_page_count()) a, (SELECT page_size FROM pragma_page_size()) b")
    try:
    ...
    return {
        ...
        "columns": columns,
        "indices": indices,
        "approximate_size_bytes": total_size // len(get_all_tables()) if get_all_tables() else 0,
    }
...
def generate_markdown_schema() -> str:
    ...
    for table in tables:
        ...
        if schema['indices']:
            md += f"**Indices:** {', '.join([f"`{idx['name']}`" for idx in schema['indices']])}\n\n"
...
def main():
    ...
    print(f"✓ Created: {md_path.relative_to(Path.cwd())}")
    ...
    print(f"✓ Created: {json_path.relative_to(Path.cwd())}")
