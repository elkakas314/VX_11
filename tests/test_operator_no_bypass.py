            ...
            if file.endswith((".ts", ".tsx", ".js", ".jsx")):
                ...
                with open(filepath, "r") as f:
                    ...
                    for port in internal_ports:
                        ...
                        pattern_fetch = rf"fetch\s*\(\s*['\"`].*(?:localhost|127\.0\.0\.1):{port}\b"
                        # Match axios/http calls
                        pattern_http = rf"(?:http|axios)\s*\.(?:get|post|put|delete)\s*\(\s*['\"`].*:{port}\b"
                        
                        if re.search(pattern_fetch, content) or re.search(pattern_http, content):
                            found_violations.append(f"{filepath}: port {port} in fetch/http call")
    
    assert not found_violations, f"Bypass violations found (fetch/http calls):\n" + "\n".join(found_violations)
...
def test_config_uses_entrypoint():
    ...
    assert "127.0.0.1:8000" in content or "http://localhost:8000" in content or "tentaculo_link:8000" in content, \
