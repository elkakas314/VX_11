...
def mock_hermes_tools():
    return {
        ...
        "local_models": [
            {"name": "model_a", "size": "7B", "loaded": True}
        ]
    }
...
async def test_vx11_overview_queries_tentaculo(mock_tentaculo_health):
    ...
    with patch('operator_backend.backend.main_v7.httpx.AsyncClient') as mock_client:
        ...
        mock_async_cm.__aenter__.return_value.get = AsyncMock(return_value=mock_response)
        mock_client.return_value.__aenter__ = mock_async_cm.__aenter__
        mock_client.return_value.__aexit__ = mock_async_cm.__aexit__
        
        response = client.get("/operator/vx11/overview", headers={"X-VX11-Token": "test_token"})
...
@pytest.mark.asyncio
async def test_shub_dashboard_queries_shubniggurath(mock_shub_health, mock_shub_metrics):
    ...
    with patch('operator_backend.backend.main_v7.httpx.AsyncClient') as mock_client:
        ...
        response = client.get("/operator/shub/dashboard", headers={"X-VX11-Token": "test_token"})
...
async def test_resources_queries_hermes(mock_hermes_health, mock_hermes_tools):
    ...
    with patch('operator_backend.backend.main_v7.httpx.AsyncClient') as mock_client:
        ...
        response = client.get("/operator/resources", headers={"X-VX11-Token": "test_token"})
