...
class PlaywrightConfig:
    ...
    SCREENSHOTS_DIR = Path(
        os.getenv("PLAYWRIGHT_SCREENSHOTS_DIR", "/app/data/screenshots")
    )
    ...
    def validate(cls) -> bool:
        ...
        write_log(
            "playwright_config", f"playwright:unknown_impl:{cls.IMPL}", level="WARNING"
        )
        return False
    ...
    def ensure_dirs(cls):
        ...
        try:
            ...
            write_log(
                "playwright_config",
                f"playwright:screenshots_dir_ready:{cls.SCREENSHOTS_DIR}",
            )
        except OSError as e:
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            async with websockets.connect(self.ws_url) as ws:
                ...
                msg = {
                    "type": "launch",
                    "params": {"headless": True, "browser": "chromium"},
                }
                ...
                if resp.get("type") == "ready":
                    ...
                    write_log(
                        "playwright_config",
                        f"playwright_remote:connected:{self.session_id}",
                    )
                    return True
                else:
                    write_log(
                        "playwright_config",
                        f"playwright_remote:handshake_failed:{resp}",
                        level="ERROR",
                    )
                    return False
            ...
            write_log(
                "playwright_config",
                "playwright_remote:websockets_not_installed",
                level="WARNING",
            )
            return False
            ...
            write_log(
                "playwright_config",
                f"playwright_remote:connect_error:{e}",
                level="ERROR",
            )
            return False
        ...
        try:
            ...
            async with websockets.connect(self.ws_url) as ws:
                ...
                resp_raw = await asyncio.wait_for(
                    ws.recv(), timeout=self.timeout_ms / 1000 + 5
                )
                resp = json.loads(resp_raw)
                ...
                if resp.get("type") == "goto_done":
                    ...
                    if ss_resp.get("type") == "screenshot_data":
                        ...
                        write_log(
                            "playwright_config",
                            f"playwright_remote:screenshot_ok:{url}",
                        )
                else:
                    result["status"] = "error"
            ...
            write_log(
                "playwright_config",
                f"playwright_remote:navigate_error:{e}",
                level="ERROR",
            )
...
class PlaywrightClientFactory:
    ...
    def create(impl: str = None):
        ...
        if impl == "stub":
            ...
            write_log(
                "playwright_config", f"playwright:unknown_impl:{impl}", level="ERROR"
            )
