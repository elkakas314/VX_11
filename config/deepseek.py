...
def call_deepseek(
    prompt: str, params: Optional[Dict[str, Any]] = None, timeout: float = 10.0
) -> Dict[str, Any]:
    ...
    if not key:
        ...
        return {
            "ok": True,
            "provider": "local",
            "result": {"summary": prompt[:512], "meta": {"len": len(prompt)}},
        }

    url = DEEPSEEK_R1_ENDPOINT
    ...
    try:
        ...
        payload = {
            ...
            "messages": [
                {
                    "role": "system",
                    "content": f"You are a helpful AI assistant specialized in {task_type}",
                },
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=timeout) as client:
            resp = await client.post(
                DEEPSEEK_R1_ENDPOINT, json=payload, headers=headers
            )
            resp.raise_for_status()
            data = resp.json()
...
def _local_reasoning_fallback(
    prompt: str, task_type: str = "reasoning"
) -> Dict[str, Any]:
