...
################################################################################
# vx11-autosync.sh
# Sincronización automática Google Docs para /home/elkakas314/vx11
# Branch: feature/ui/operator-advanced (rama principal)
# ÚNICO REPO LOCAL - sincroniza TODO el contenido local al remoto
################################################################################

set -o pipefail

# Configuration
REPO_PATH="/home/elkakas314/vx11"
BRANCH="feature/ui/operator-advanced"
REMOTE="vx_11_remote"
SYNC_INTERVAL=60  # segundos (configurable)
LOG_FILE="/tmp/vx11-autosync.log"
GITHUB_PAT="${GITHUB_PAT:-}"

# Inicializar log
init_log() {
    echo "[$(date -u)] ===== AUTOSYNC START =====" > "$LOG_FILE"
...
check_token() {
    if [ -z "$GITHUB_PAT" ] && [ -f "$REPO_PATH/tokens.env" ]; then
        export GITHUB_PAT=$(grep "GITHUB_PAT=" "$REPO_PATH/tokens.env" 2>/dev/null | cut -d'=' -f2 | tr -d '\n' | tr -d '"' | tr -d "'")
    fi
...
setup_remote() {
    ...
    # Configure credential helper for HTTPS push
    ...
    while true; do
        ...
        if [ -f "$REPO_PATH/.git/MERGE_HEAD" ] || [ -f "$REPO_PATH/.git/REBASE_MERGE" ]; then
            if [ "$conflict_pause" -eq 0 ]; then
                ...
                log_msg "After resolving, autosync will resume automatically."
                conflict_pause=1
                echo "🚨 CONFLICTO EN SINCRONIZACIÓN: Ver /tmp/vx11-autosync.log" >> "$LOG_FILE"
            fi
        ...
        if [ -n "$(git status --porcelain)" ]; then
            ...
            if git add -A 2>&1 | grep -q "error"; then
                ...
                COMMIT_MSG="autosync: $(date '+%Y-%m-%d %H:%M:%S UTC')"
...
trap 'log_msg "AUTOSYNC STOPPED (PID: $$)"; exit 0' SIGTERM SIGINT
