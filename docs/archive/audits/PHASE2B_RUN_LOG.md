...
### Lessons Learned
- Model availability varies; conservative fallback strategy prevents complete failure
- Dev vs prod paths require explicit strategy (local data/ + env var + documentation)
- Python 3.10 f-string limitations require careful syntax
- Path handling in subprocess scripts needs simplification (avoid relative_to)

---

## Next Steps (Phase 3+)

1. **FASE 4 (Deferred to Phase 3): Switch CLI Concentrator Scoring**
   - Implement Copilot-first priority weighting
   - Add fallback providers (Gemini, Codex, Qwen) from Hermes registry
   - Integrate circuit breaker per provider + coste/latencia scoring
   - Implement explainability logging (why provider X chosen)

2. **FASE 5 (Phase 3): FLUZO Signal Integration**
   - Connect to power/thermal sensors for ultra-low-memory mode
   - Adjust model rotation based on system load
   - Implement graceful degradation (use CLI-first when local models warm)

3. **Deployment (Phase 3):**
   - Activate Playwright sidecar: `docker-compose -f docker-compose.yml -f docker-compose.playwright.yml up -d`
   - Spin up all 9 modules + optional processors
   - Deploy metrics dashboard + health monitoring
